...
// UTILS
import { searchOSM } from "../../utils/osmAutocomplete";
import { getDistanceAndDuration } from "../../utils/osrmDistance";
...
const BookRide = () => {
  ...
  const background = useBreakpointValue({
    base: "none",
    lg: "linear-gradient(135deg, #6B73FF 0%, #000DFF 100%)",
  });
  const heading = useBreakpointValue({ base: "brand.500", lg: "white" });
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      const res = await searchOSM(address.pickup);
      if (res.length) {
        const place = res[0];
        const location = {
  ...
  // Debounce drop search
  useEffect(() => {
    const timer = setTimeout(async () => {
      const res = await searchOSM(address.destination);
      if (res.length) {
        const place = res[0];
        const location = {
          lat: Number(place.lat),
  ...
  const search = async () => {
    ...
    const result = await getDistanceAndDuration(pickupData, dropData);
    if (!result) return;
    ...
    if (distanceKm > 50) {
      toast({
        title: "Trip is out of serviceable area.",
        description: `Your trip is ${distanceKm.toFixed(
          1
        )}kms long. Please choose a trip below 50kms.`,
        status: "error",
        position: "top",
        isClosable: true,
      });
  ...
  return (
    <>
      ...
      <Flex mt="2rem" mx="1rem" flexDir="column" align="center" gridGap="1rem">
        <FormControl>
